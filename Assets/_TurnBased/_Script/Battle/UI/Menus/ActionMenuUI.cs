...
public class ActionMenuUI : MonoBehaviour
{
    ...
    private void OnSkillClicked(ScriptableSkill clickedSkill, Button clickedButton)
    {
        ...
        if (IsFriendlyTargetSkill(clickedSkill) && clickedSkill.targetScope == TargetScope.Single)
        {
    ...
    {
        foreach (Button button in _activeSkillButtons)
        {
    ...
    private void CancelAllySelection()
    {
        ...
        {
            casterPanel.SetHighlighted(false);
            if (preAllySelectSkill != null) casterPanel.SetIntentText(preAllySelectSkill.skillName);
        }

        if (EventSystem.current != null && preAllySelectButton != null)
