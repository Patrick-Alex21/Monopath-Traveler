...
public class TargetingSystem : MonoBehaviour
{
    ...
    private bool isPreviewingAll = false;
    private readonly List<GameObject> previewArrows = new List<GameObject>();
    ...
    private void HandleWorldClick()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit))
        {
            ...
            {
                if (isTargeting && !isPreviewingAll)
                {
                    currentTargetIndex = CharacterManager.Instance.ActiveEnemies.IndexOf(clickedEnemy);
                    currentTarget = clickedEnemy.gameObject;

                    if (BattleManager.Instance != null &&
                        BattleManager.Instance.State == BattleState.SelectTarget)
                    {
                        BattleManager.Instance.ApplyTargetToPlanningHero(clickedEnemy);
                    }
                    else if (BattleManager.Instance != null &&
                             BattleManager.Instance.State == BattleState.HeroTurn)
                    {
    ...
    public void StartTargeting(CharacterBase previousTarget = null, bool showImmediately = true, bool autoHide = true)
    {
        ...
        {
            if (hideTimerCoroutine != null) { StopCoroutine(hideTimerCoroutine); hideTimerCoroutine = null; }
            SetArrowVisible(false);
        ...
        public void PreviewAllEnemies()
    {
        ...
        {
            TargetHighlight highlight = enemies[i].GetComponent<TargetHighlight>();
            ...
            if (arrowIndicatorPrefab != null)
            {
                ...
                arrow.transform.SetParent(enemies[i].transform, false);
                TargetIndicator indicator = arrow.GetComponent<TargetIndicator>();
                if (indicator != null) indicator.SetPivot(arrowFallbackOffset);
                else arrow.transform.localPosition = arrowFallbackOffset;
    ...
    public void PreviewSingleTarget()
    {
        ClearPreviewArrows();
        isPreviewingAll = false;
        ...
        if (currentTargetIndex < 0 || currentTargetIndex >= enemies.Count) currentTargetIndex = 0;

        for (int i = 0; i < enemies.Count; i++)
        {
            TargetHighlight highlight = enemies[i].GetComponent<TargetHighlight>();
            if (highlight != null) highlight.SetHighlight(i == currentTargetIndex);
        }

        EnsureArrowExists();
        if (currentArrow != null)
        {
            currentArrow.transform.SetParent(enemies[currentTargetIndex].transform, false);
            TargetIndicator indicator = currentArrow.GetComponent<TargetIndicator>();
            if (indicator != null) indicator.SetPivot(arrowFallbackOffset);
            else currentArrow.transform.localPosition = arrowFallbackOffset;
            SetArrowVisible(true);
    ...
    public void ClearPreview()
    {
        ...
        {
            SetArrowVisible(false);
            if (enemies != null && currentTargetIndex >= 0 && currentTargetIndex < enemies.Count)
            {
                TargetHighlight highlight = enemies[currentTargetIndex].GetComponent<TargetHighlight>();
                if (highlight != null) highlight.SetHighlight(false);
            }
        }
    }

    private void ClearPreviewArrows()
    {
    ...
    public CharacterBase GetCurrentTarget()
    {
        ...
        if (currentTargetIndex < 0 || currentTargetIndex >= enemies.Count)
            currentTargetIndex = 0;
        return enemies[currentTargetIndex];
    }

    private void OnTargetingPerformed(InputAction.CallbackContext ctx)
    {
        if (!isTargeting || isPreviewingAll) return;
    ...
    private void OnSubmitPerformed(InputAction.CallbackContext ctx)
    {
        ...
        if (BattleManager.Instance != null && BattleManager.Instance.State == BattleState.SelectTarget) return;
        ConfirmTarget();
    }
    ...
    private void ChangeTarget(int direction)
    {
        ...
        currentTarget = selectedEnemy.gameObject;
        if (BattleManager.Instance != null && BattleManager.Instance.State == BattleState.SelectTarget)
        {
            BattleManager.Instance.ApplyTargetToPlanningHero(selectedEnemy);
        }
        else if (BattleManager.Instance != null && BattleManager.Instance.State == BattleState.HeroTurn)
            BattleManager.Instance.ApplyTargetToAllHeroes(selectedEnemy);
